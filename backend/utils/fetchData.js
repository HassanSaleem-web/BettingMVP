...
function impliedFromDecimals({ H, D, A }) {
  ...
  const pA = 1 / A;
  const sum = pH + pD + pA;
  return { H: pH / sum, D: pD / sum, A: pA / sum };
}
...
function averageDecimals(list) {
  ...
  const sum = list.reduce((acc, x) => ({
    H: acc.H + x.H, D: acc.D + x.D, A: acc.A + x.A
  }), { H: 0, D: 0, A: 0 });
  return { H: sum.H / n, D: sum.D / n, A: sum.A / n };
}
...
async function fetchAndStoreFixtures() {
  ...
  for (const fx of oldFixtures) {
    ...
    if (["FT", "AET", "PEN", "CANC", "PST", "ABD"].includes(status)) {
      await Fixture.deleteOne({ fixtureId: fx.fixtureId });
    }
  }

  // STEP 2: Fetch new fixtures
...
async function fetchOddsForFixtures() {
  ...
  for (const f of fixtures) {
    ...
    if (Imp_B365 || avg) {
      ...
      setDoc.odds = { ...mappedOdds, fetchedAt: new Date() };

      await Fixture.updateOne({ fixtureId }, { $set: setDoc });
    }
  }
}
...
async function fetchAndStoreLast5ForStoredFixtures({ forceRefresh = false, rateLimitMs = 600 } = {}) {
  ...
  for (const fx of fixtures) {
    ...
    const setDoc = {
      ...
      form: {
        home: { wins: homeForm.wins, draws: homeForm.draws, losses: homeForm.losses },
        away: { wins: awayForm.wins, draws: awayForm.draws, losses: awayForm.losses },
        formFetchedAt: new Date()
      }
    };

    await Fixture.updateOne({ fixtureId: fx.fixtureId }, { $set: setDoc });
  }
}
